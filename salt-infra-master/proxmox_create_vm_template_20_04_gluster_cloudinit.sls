...
import_image:
  cmd.run:
    - name: |
        qm importdisk {{ vm_template_id }} /var/lib/vz/template/iso/{{image_name}} {{gluster_volume}} 


attach_disk:
  cmd.run:
    - name: |
        qm set {{ vm_template_id }} -scsihw virtio-scsi-pci -virtio0 {{gluster_volume}}:vm-{{ vm_template_id }}-disk-0
...
set_cloudinit_drive:
  cmd.run:
    - name: |
        qm set {{ vm_template_id }} -ide2 {{gluster_volume}}:cloudinit
# 8GB
resize_drive:
  cmd.run:
    - name: |
